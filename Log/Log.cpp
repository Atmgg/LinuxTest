...
CLog::CLog(const char *filePath, LOG_LEVEL level):m_logFilePath(filePath), m_LogLevel(level), m_showLogFlag(true)
...
size_t CLog::GetLogFileSize()
{
	...
	streampos curPos = m_fileOut.tellg();  // 当前写文件指针位置

	m_fileOut.seekg(0, ios::end);  // 将写文件指针指向文件末尾 
	streampos pos = m_fileOut.tellg(); // 获取写文件指针的位置 该值即为文件大小
	m_fileOut.seekg(curPos);   // 恢复写文件指针位置
...
// 写日志  level: 该条日志级别  pLogText: 日志内容
void CLog::WriteLog(const char *pLogText, LOG_LEVEL level)
{
    if (m_openSuccess && pLogText && level <= m_LogLevel)
    {
        string logText = pLogText;
...
void CLog::WriteLog(LOG_LEVEL level, const char *pLogText, ...)
{
    ...
    vsnprintf(logText, LOG_LINE_MAX - 1, pLogText, args);
    WriteLog(logText, level);
}
...
void CLog::ClearLogFile()
{
    ...
    CloseLogFile(); // 关闭流
    m_fileOut.open(m_logFilePath.c_str(), ios::out | ios::trunc); // 以截断方式打开文件
    m_openSuccess = m_fileOut.is_open();
}
